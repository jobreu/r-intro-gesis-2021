...
Generally, there the logic of `[index_number]` is used in `R` to access only a subset of information in an object, no matter if we have vectors or data frames.
...
```{r read-stata-example_output, eval = FALSE}
gp_covid
```

.tinyish[
```{r read-stata-example_output-print, echo = FALSE}
gp_covid
```
]
