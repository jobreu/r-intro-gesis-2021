...
In the previous session, we learned how to define missing values. Here we will use the `set_na()` function from the `sjlabelled` package.
...
## Using the function `across()` logical conditions
...
**Sometimes, things are a bit more complicated. Simple recoding is insufficient when we need to base new variables based on the values of old variable(s). Such procedures are called conditional transformations.**
...
For example, we have learned from the `sum()` function and one issue with that is that it prints `NA` by default when missing values are in the data. So we always have to set the `na.rm = TRUE` option. A function to circumvent this repetition would be: 
