...
After recoding a set of values as `NA` for one variable, let's now do the same for the whole `gp_covid` dataframe. This time, however, we do not want to recode 98 as `NA` as it is a valid value for the `id` variable.
```
