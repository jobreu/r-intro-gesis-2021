...
After running a few regression analyses in the previous exercises, we will now explore some options for reporting the results. This is also a bit of a preparation for the session on reporting with `R Markdown` tomorrow.
...
In addition to the `parameters` package, which we have already used in the previous exercises on regression analysis, this time we also need the following packages: `stargazer`, `report`, `broom`. 
...
Now we also want some more interesting information on the results of our model (including confidence intervals and p-values) in a nice tabular format.
```

```{block, opts.label = "clues"}
We can use a function from the `parameters` package here.
```
...
For further use in a publication, we also want to create a typical regression table using the `stargazer` package (in plain text format).
```
...
```{block, box.title = "5", box.body = list(fill = "white"), box.icon = "fa-star"}
Of course, we can't write a paper that just consists of tables and plots (though some would surely very much appreciate that). We also need to produce some actual text. We all now the "You should be writing" memes, so we're in luck that `R` can also help us here. Let's use a function that produces some model language describing the results of out regression model. 
```
